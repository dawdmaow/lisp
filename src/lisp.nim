when not defined(js):
  {.fatal: "This can only be run on the JS backend.".}

import
  std/[sequtils, strutils, parseutils, tables, sugar, enumerate, hashes, algorithm, dom]
import dawid/dawid
...
const browser = defined(js) and not defined(nodejs)
...
type
  Magic = enum
    ...
    mContinue = "continue"
    mBreak = "break"
    mJoinLists = "join-lists" # mEchoNoNewline = "echo-raw"
  ...
  TokenKind = enum
    tkSymbol
    tkNumber = "number-type"
    tkString = "string-type"
    tkOpen
    tkClose
    tkKeyword
    tkBool
  ...
  Node = ref object
    ...
    of nkClassType:
      ...
      classMethods: Table[string, Node]
        # stores function nodes, not method nodes, because the class know which object the method will belong to anyway. it's just a prototype for a method node to instantiate. TODO: Consider changing the name to something else.
    of nkClassObj:
      classObjType: Node
      classObjFields: Table[string, Node]
      classObjMethods: Table[string, Node]
        # TODO: strings seem a little excessive for memory usage, we should be able to get away with integers/hashes.
...
template assert(t: Token or Node, cond: bool, s: string) {.dirty.} =
  if not cond:
    ...
    raiseAssert "\n\nError in lisp script, line " & $inst.line & " (script line " &
      $t.line & "): " & s
...
const mustBeEvaluatedBeforeComparison = {nkSymbol, nkDot, nkMember}
...
proc copyTree(n: Node): Node =
  ...
  of nkClassObj:
    ...
    result.classObjFields = collect(
      for k, v in n.classObjFields:
        {k: copyTree v}
    )
    result.classObjMethods = collect(
      for k, v in n.classObjMethods:
        {k: copyTree v}
    ) # TODO: callstack size shittery when uncommented
  of nkVoidType:
  ...
  of nkTableType:
    result = Node(
      kind: nkTableType,
      tableKeyType: copyTree(n.tableKeyType),
      tableValueType: copyTree(n.tableValueType),
    )
  of nkArrayType:
  ...
  of nkFunction:
    ...
    result.funcBody = copyTree(n.funcBody)
      # TODO: callstack size shittery when uncommented
...
template temporaryStack(kindx: StackKind, code) =
  ...
  try:
    ...
    s.stacks.delete s.stacks.high
      # We want to make sure that the stack is balanced because errors might be catchable from the lisp code itself (in the future).
...
proc tokenize(s: string): seq[Token] =
  ...
  const atomicTypes = collect(
    for it in AtomicType:
      $it
  )
  ...
  while i <= s.high:
      ...
      while true:
        ...
        if s[i] == '\\':
          ...
          of 't':
            ...
            raiseAssert "Invalid escape sequence: " & $s[i]
          inc i
    ...
    of Digits:
      ...
      #   add Token(kind: tkNone, line: line)
        ...
        add Token(
          kind: tkAtomicType, atomicType: parseEnum[AtomicType](tmp), line: line
        )
...
proc expectValidEnumValue(n: Node) =
  ...
  n.assert n in n.enumType.enumTypeValues,
    "Invalid enum value: " & $n & ". Valid values: " &
...
proc canConvertToEnum(n: Node, enumType: Node): bool =
  n.assert n.kind != nkSymbol:
    "Node must be evaluated before conversion"
...
const typeKinds = {
  nkEnumType, nkAtomicType, nkObjectType, nkTableType, nkAnyType, nkVoidType,
  nkClassType, nkArrayType,
}
...
proc canConvertToType(n: Node, newType: Node): bool =
  ...
  assert newType.kind in typeKinds:
    "Type node expected, got: " & $n
  ...
  of nkTableType:
    if n.kind == nkTable:
      result = true
      for k, v in n.tableVal:
        if not k.canConvertToType(newType.tableKeyType) or
            not v.canConvertToType(newType.tableValueType):
          result = false
          break
    else:
      result = false
...
proc convertValueToType(val, typ: Node): Node =
  ...
  val.assert val.canConvertToType typ:
    "Can't convert value from type " & $val.getType & " to type " & $typ & ": " &
      val.repr
  ...
  of nkClassType:
    ...
    result = val.copyTree
      # TODO: questionable, but we don't want to change the type of an existing node. or any other of it's properties. Let's figure out a way of having a node kind that wraps other nodes without copying them such that their nominal type is changed.
...
const binaryKeywords = {
  mAssign, mVar, mTest, mPlus, mEqualPlus, mMinus, mEqualMinus, mEqual, mOr, mAnd,
  mRange, mLesserOrEq, mLesser, mGreaterOrEq, mGreater,
}
      ...
      if n.kids.len == 3:
        if n.kids[0].kind == nkKeyword:
          if n.kids[0].keyword in binaryKeywords:
            n.assert false,
              "Binary operator must be used like this: (a + b) instead of (+ a b)"
...
proc buildAst(tokens: openarray[Token]): Node =
  result = Node(
    kind: nkList, line: 1, kids: @[Node(kind: nkKeyword, keyword: mBlock, line: 1)]
  )
  ...
  for t in tokens:
    ...
    of tkTag:
      t.assert t.tag notin currentTags,
        "Tag defined more than once for the same node: " & $t.tag
...
proc evaluateKeywordCall(s: State, n: Node): Node {.nimcall.} =
  ...
  of mArrayType:
    ...
    args[0].assert valueType.kind in typeKinds:
      "Expected a type"
  ...
  of mTableType:
    ...
    args[0].assert keyType.kind in typeKinds:
      "Expected a type"
    args[1].assert valueType.kind in typeKinds:
      "Expected a type"
  ...
  of mExcept, mFinally:
    n.assert false:
      "Except and finally blocks are not allowed outside of a try block"
  of mTry:
    n.assert args.len >= 1
    ...
    for a in args:
      ...
      if a.kind == nkList:
        a.assert a.kids.len > 0:
          "Empty list is invalid"
        case a[0].keyword
        of mExcept:
          a.assert exceptBlock == nil:
            "Only one except block allowed"
          a.assert finallyBlock == nil:
            "Finally block must be the last one"
      ...
      if not didFind:
        a.assert exceptBlock == nil:
          "Except block must be declared after the body"
        a.assert finallyBlock == nil:
          "Finally block must be declared after the body"
        body.kids.add a
  ...
  of mTemplate:
    ...
    result = Node(
      kind: nkTemplate, templateParams: params.kids.mapIt(it.sym), templateBody: body
    )
  of mContains:
    ...
    for i, item in items.kids:
      temporaryStack skBlock:
        try:
          ...
          if keySym != nil:
            s.stacks[^1].symbols[keySym.sym] = ast:
              i.float
          discard s.evaluate body
        except Break:
          break
  ...
  of mNotEqual:
    assert args.len == 2
    result = ast:
      s.evaluate(args[0]) != s.evaluate(args[1])
  of mToStr:
    assert args.len == 1
    result = ast:
      $s.evaluate(args[0])
  ...
  of mIsStr:
    assert args.len == 1
    result = ast:
      s.evaluate(args[0]).isKind atString
  of mIsList:
    assert args.len == 1
  ...
  of mAssign:
    ...
    args[1].assert lval.kind notin typeKinds:
      "Rebinding types is not allowed"

    let converted = rval.copyTree.convertValueToType(ltype)
      # Copying the tree to avoid changing the type of the original value. TODO: This changes ref semantics to struct semantics (implicitly), which is probably bad.

    # dump lval[]
    lval[] = converted[]
      # FIXME: This is a very bad way to copy. Reference/struct semantics should be dependant on the type of the rvalue, and n ested. Use a dedicated (copy) for actually copying values.
  ...
  of mClass:
    ...
    let name = args[0]
    name.expectKind nkSymbol
      # TODO: validate that the symbol is not already defined in current scope, using a proc that should then be used everywhere else in the old code!
    ...
    for a in args[2 ..^ 1]:
      ...
      of "field":
        ...
        fieldName.assert fieldName.sym notin result.classFields,
          "Field already defined: " & fieldName.sym
      ...
      of "method":
        ...
        methodName.assert methodName.sym notin result.classMethods,
          "Method already defined: " & methodName.sym
  ...
  of mType:
    ...
    for f in fields.kids:
      ...
      name.assert name.sym notin result.objectTypeFields,
        "Field already defined: " & name.sym

      let typ = s.evaluate f.kids[1]
      typ.assert typ.kind in typeKinds:
        "Expected a type: " & $typ.kind
  ...
  of mLambda:
    ...
    args[1].assert funcReturnType.kind in typeKinds,
      "Expected a return type: " & $funcReturnType
  ...
  of mVar:
    ...
    n.assert not s.stacks[^1].symbols.hasKey(sym.sym),
      "Symbol already defined: " & sym.sym

    let value = s.evaluate args[1]
    # echo ("?", args[1], value)
    n.assert value.kind != nkSymbol:
      "This is why Common Lisp uses keywords instead of symbols for this purpose."
  ...
  of mIf:
    ...
    n.assert condition.getAtomicValueKind == atBool,
...
proc evaluate(s: State, n: Node): Node =
  ...
  case n.kind
  of nkKeyword, nkFunction, nkTable, nkAtomicType, nkObjectType, nkObject, nkAtomic,
      nkTemplate, nkEnumType, nkTableType, nkArrayType, nkAnyType, nkVoidType,
      nkVoidValue, nkClassObj, nkClassType: #, nkMacro:
  ...
  of nkDot:
    for i, sym in enumerate n.dot.split("."):
      ...
      n.assert result != nil,
        "Dot call symbol not found: " & sym & " from " & n.dot & " where stacks " &
          $s.stacks
  of nkList:
    ...
    of nkAtomicType, nkObject, nkAtomic, nkTableType, nkArrayType, nkAnyType,
        nkVoidType, nkVoidValue, nkClassObj:
      n.assert false, "First element of list is not callable: " & $n & ", " & $f
    of nkEnumType:
      n.assert args.len == 1,
        "Enum type variable declaration requires exactly 1 argument"
    ...
    of nkClassType:
      ...
      for a in args:
        ...
        let fieldName = a[0]
        fieldName.expectKind nkSymbol
        fieldName.assert fieldName.sym in allFields,
          "Field not found in class type: " & fieldName.sym
        fieldName.assert fieldName.sym notin obj.classObjFields,
          "Field already defined: " & fieldName.sym

        let fieldValue = s.evaluate a[1]
        fieldValue.assert fieldValue.canConvertToType allFields[fieldName.sym],
          "Invalid field type"
    ...
    of nkObjectType:
      n.assert args.len == 1,
        "Object type variable declaration requires exactly 1 argument"
      let fields = args[0]
      ...
      for a in fields.kids:
        ...
        let key = a.kids[0]
        key.assert key.kind == nkSymbol,
          "Object field names must be symbols, got: " & $key.kind
        key.assert key.sym notin result.objectFields,
          "Field already defined: " & key.sym
        ...
        value.assert value.getType == fieldTyp:
          "Type mismatch for field `" & key.sym & "`. Expected " & $fieldTyp & ", got " &
            $value.getType
    ...
    of nkTemplate:
      ...
      proc update(
          s: State,
          n: var Node,
          templateArgs: Table[string, Node],
          parent: Node,
          indexInParent: int,
      ) {.nimcall.} =
        assert not n.isNil
        ...
        if tkUnquote in n.tags:
          ...
          n.assert n.sym in templateArgs:
            "Template call has no argument to unquote-splicing: " & n.sym
          n.assert parent != nil:
            "Can't unquote-splice at the top level"
          n.assert not parent.isNil:
            "Can't unquote-splice at the top level"
          n.assert parent.kind == nkList:
            "Can't unquote-splice outside of a list"
          n.assert indexInParent in parent.kids.low .. parent.kids.high:
            "Invalid indexInParent"
          n.assert nkList == nkList:
            "Can't unquote-splice a non-list"
          ...
          parent.kids.delete indexInParent
            # Removing this node. This somehow makes our `n: var Node` value nil *right after the statement executes*! (This is why we need to copy the kids for use below.)
          # parent.kids.insert kids, 0 # Replacing this node's position with it's own children.
          parent.kids.insert argToSplice.kids, indexInParent
            # Replacing this node's position with it's own children.
        elif n.kind == nkList:
          var i = n.kids.low
          while i <= n.kids.high:
            # The length can change during iteration because we insert insert quote-splicing exists.
            n.expectKind nkList
            ...
            if not typ.isNil:
              ...
              if typ.kind != nkAnyType:
                n.assert val.getType == typ:
                  "Type mismatch for parameter " & param & ". Expected " &
                    $f.funcTypes[i] & ", got " & $val.getType
...
const scripts = block:
  ...
  for t in tests.mitems:
    t.kind = testScript
  for e in examples.mitems:
    e.kind = exampleScript
...
when browser:
  ...
  const firstScriptName = "core"

  var
    terminalInitialized = false
    currentScriptName = firstScriptName
      # TODO: use the Script object instead of mulitple globals!
  ...
  discard setInterval(resize, 100)
    # TODO: Karax doesn't call `fit` properly so this is a workaround.
        ...
        tdiv(class = "flex gap-1 h-screen p-3 gap-3"):
          tdiv(class = "flex-1 h-full flex flex-col gap-4"):
            tdiv(class = "flex gap-4"):
              select(
                id = "scripts-select",
                class = "w-full bg-white/10 text-white p-2 rounded-lg appearance-none",
              ):
                for s in scripts:
              ...
              button(class = "btn bg-blue-500 px-4 rounded-lg py-2"):
                ...
                proc onClick() =
                  ...
                  dump code
                  runScript currentScriptName, $code, currentScriptModified
                    # TODO: optimize string fuckery

            tdiv(
              class =
                "h-full w-full bg-black resize-none bg-white/10 text-xs rounded-lg",
              id = "input",
            ):
